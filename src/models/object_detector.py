...
class ObjectDetector:
    ...
    def __init__(
        ...
        self.model = torch.hub.load(
            repo_or_dir=model_name, model=model_version, pretrained=True
        )
    ...
    def extract_objects(self, images: List[str]) -> pd.DataFrame:
        ...
        :returns: All the boxes (as dataframe), columns: img_name,x1,x2,y1,y2,yolo_confidence,yolo_class_id,yolo_class_name.
        """
        logging.info(f"[Object Detector] Prediction on {len(images)} images")
...
if __name__ == "__main__":
    ...
    results = model.extract_objects(
        [
            os.path.join("data", "train", "images", str(i).rjust(4, "0") + ".jpg")
            for i in range(1)
        ]
    )
